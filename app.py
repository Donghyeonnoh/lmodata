        ...
        if st.button("🚀 분석 요청하기!"):
            if not question:
                ...
                with st.spinner("🧠 DAVER가 스스로 생각하고 분석 중입니다..."):
                    # --- [v4.2 핵심 수정!] 더 일반적이고 구조적인 예시를 제공하는 최종 프롬프트 ---
                    total_rows = len(df_filtered)
                    
                    prompt = f"""
                    당신은 Python Pandas와 Streamlit을 전문적으로 다루는 AI 데이터 분석가입니다.
                    당신의 임무는 [사용자 질문]을 분석하고, [분석할 데이터]의 구조를 참고하여 아래 [지침]에 따라 답변을 생성하는 것입니다.

                    **[지침]**
                    1.  **'생각의 과정' 작성:** [사용자 질문]을 해결하기 위한 계획을 한글로 1, 2, 3... 단계별로 세웁니다. 이 계획은 매우 구체적이어야 합니다. 어떤 컬럼을 사용할 것인지, 어떤 Pandas 함수를 적용할 것인지 명시해야 합니다.
                    2.  **'최종 실행 코드' 작성:** 위 '생각의 과정'에서 세운 계획을 그대로 실행하는 Python 코드 블록만 생성합니다. 결과는 반드시 Streamlit(st)을 사용해 화면에 출력해야 합니다.
                    3.  **출력 형식 준수:** 답변은 `## 생각의 과정`과 `## 최종 실행 코드` 두 제목으로만 구성되어야 합니다. 코드 외에 다른 설명이나 주석은 절대 포함하지 마세요.

                    **[분석할 데이터 정보]**
                    * **총 행 개수:** {total_rows}
                    * **컬럼 목록:** {', '.join(df_filtered.columns)}
                    * **데이터 샘플 (상위 3개):**
                        {df_filtered.head(3).to_csv(index=False)}
